...
### 状态模式
* 背景：对象的行为依赖于它的状态：状态变了，行为也变了。此时可以把与状态有关的行为封闭到一个个状态类中。
* 实现：3个类：抽象状态类，具体状态类，环境类。环境类有一个状态对象作为当前状态。状态的转换代码可以放在每个具体对象中，这时每个具体对象通过调用环境类的setState方法来转换状态。也可以把状态的转换代码放在环境类中，由环境类来负责。此时环境类要持有所有的具体对象，并有一个域currentState来指向其中的一个个具体对象。
